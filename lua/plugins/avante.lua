return {
    ...
    opts = {
        ...
        providers = {
            ...
            claude_fast = {
                ...
                extra_request_body = { max_tokens = 8192 }, -- 8k per Vertex model card
            },
        },
        windows = {
    ...
    config = function(_, opts)
        ...
        local current = opts.provider -- "claude" by default
        local main, fast = "claude", "claude_fast"

        local function switch(to)
            vim.cmd("AvanteSwitchProvider " .. to)
            current = to
            local label = (to == fast) and "Fast Model: Haiku 3" or "Slow Model: Sonnet 4"
            vim.notify("Avante provider → " .. label, vim.log.levels.INFO, { title = "Avante" })
        end

        vim.keymap.set("n", "<leader>cm", function()
            switch(current == main and fast or main)
        end, { desc = "avante: toggle mini model" })
    end,
    dependencies = {
